...
public sealed class ConditionController : ControllerBase
{
    ...
    public ConditionController(FhirUpstreamClient upstream)
    {
        _upstream = upstream;
    }
    ...
    public async Task<IActionResult> Search([FromQuery] Guid? patient, [FromQuery] Guid? encounter, [FromQuery(Name = "_count")] int count = 50, CancellationToken cancellationToken = default)
    {
        ...
        return encounters.IsSuccess
            ? Ok(FhirJson.Bundle(Request, "Condition", encounters.Value.SelectMany(FhirJson.Conditions)))
            : StatusCode((int)encounters.StatusCode, FhirJson.OperationOutcome(encounters.Error ?? "Condition search failed."));
    }
