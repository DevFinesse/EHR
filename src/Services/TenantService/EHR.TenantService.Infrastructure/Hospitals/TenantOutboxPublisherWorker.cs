...
{
    public TenantOutboxPublisherWorker(string connectionString, IEventBus eventBus, ILogger<TenantOutboxPublisherWorker> logger)
        : base(new DbContextOptionsBuilder<TenantDbContext>().UseNpgsql(connectionString).Options, eventBus, logger)
    {
    }
